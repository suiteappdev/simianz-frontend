...
function appCtrl($http, $scope, $rootScope, $timeout, $storage, $account, DTOptionsBuilder) {
    ...
    $scope.addRemove = function(){
        ...
        for(x in $rootScope.items)
            if($rootScope.items[x]._id === this.record._id)
                {
                    
                 $rootScope.items.splice(x,1);

                 return false;
                }


         $rootScope.items.push(this.record);
